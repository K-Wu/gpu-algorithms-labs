...
__global__ void gpu_normal_kernel(float *in_val, float *in_pos, float *out, int grid_size, int num_in) {
  //@@ INSERT CODE HERE
  // each thread deal with one grid point
...
__global__ void gpu_cutoff_binned_kernel(int *bin_ptrs, float *in_val_sorted, float *in_pos_sorted, float *out, int grid_size,
  ...
  // int  influentialBinBeg = 0;
  // int  influentialBinEnd = NUM_BINS;
  
  const int firstElementIdxInBlock = (blockIdx.x * blockDim.x) < (grid_size-1)? (blockIdx.x * blockDim.x): (grid_size-1);
  const int lastElementIdxInBlock  = ((blockIdx.x + 1) * blockDim.x - 1) < (grid_size-1)? ((blockIdx.x + 1) * blockDim.x - 1): (grid_size-1);


  int influentialBinBeg = firstElementIdxInBlock *NUM_BINS / grid_size;
  int influentialBinEnd = lastElementIdxInBlock *NUM_BINS / grid_size + 1;

  while (influentialBinBeg>0 && (in_pos_sorted[bin_ptrs[influentialBinBeg]-1]-firstElementIdxInBlock)<cutoff) { //check if the bin right before influentialBinBeg should be included
    influentialBinBeg--;
  }
  while(influentialBinEnd<NUM_BINS && (in_pos_sorted[bin_ptrs[influentialBinEnd]]-lastElementIdxInBlock)<cutoff) { //check if the bin right after (influentialBinEnd - 1) should be included
    influentialBinEnd++;
  }

  if (outIdx < grid_size) {
    for (int influentialBinIdx = influentialBinBeg; influentialBinIdx < influentialBinEnd; influentialBinIdx++) {
      const int inSortedIdxBeg = bin_ptrs[influentialBinIdx];
      const int inSortedIdxEnd = bin_ptrs[influentialBinIdx + 1];
      for (int inSortedIdx = inSortedIdxBeg; inSortedIdx < inSortedIdxEnd; inSortedIdx++) {
        const float in_val2 = in_val_sorted[inSortedIdx] * in_val_sorted[inSortedIdx];
        const float dist    = in_pos_sorted[inSortedIdx] - (float) outIdx;
        if (dist * dist < cutoff2) {
          out[outIdx] += in_val2 / (dist * dist);
        }
      }
  ...
  *******************************************************************************/
  ...
  int eval(const int num_in, const int max, const int grid_size) {
    ...
    if (mode == Mode::GPUBinnedCPUPreprocessing) {
      ...
      cpu_preprocess(in_val_h.data(), in_pos_h.data(), in_val_sorted_h.data(), in_pos_sorted_h.data(), grid_size, num_in,
                     bin_counts_h.data(), bin_ptrs_h.data());
      timer_stop();
    }
